...
log_step "Recreating kind Cluster"
run "kind delete cluster || true"
run "kind create cluster --config '$SCRIPT_DIR/kind-config.yaml'"

log_step "Installing Cilium (CNI & Ingress)"
run "helm repo add cilium https://helm.cilium.io/"
run "helm repo update"
# Install Cilium with Ingress and Hubble enabled
run "helm install cilium cilium/cilium --version 1.16.0 \
...
log_step "Waiting for Cilium"
run "kubectl rollout status deployment/cilium-operator -n kube-system"
run "kubectl rollout status daemonset/cilium -n kube-system"
